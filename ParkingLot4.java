...
public class ParkingLot4 {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ParkingLot4 window = new ParkingLot4();
					window.frameMSL.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frameMSL = new JFrame();
		frameMSL.getContentPane().setBackground(new Color(135, 206, 250));
		frameMSL.setBounds(100, 100, 1104, 688);
		frameMSL.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frameMSL.setLocationRelativeTo(null);
		frameMSL.getContentPane().setLayout(null);
		...
		frameMSL.getContentPane().add(lblNewLabel);
		
		JTabbedPane tabbedPane = new JTabbedPane(JTabbedPane.TOP);
		tabbedPane.setBounds(10, 84, 1072, 558);
		frameMSL.getContentPane().add(tabbedPane);
		...
		Parkbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg) {
				if(comboBox_1.getSelectedItem()!="CAR" || comboBox_1.getSelectedItem()!="MOTORCYCLE" || comboBox.getSelectedItem()=="MANAGEMENT STAFF") {
					JOptionPane.showMessageDialog(Parkbutton, "PLEASE CHANGE OCCUPATION/VEHICLETYPE/PARKINGTYPE. TQ.","NOT ALLOWED TO PARK HERE",JOptionPane.OK_OPTION);
				}
				else {
		...
		frameMSL.getContentPane().add(Logoutbutton);
			}
	}
