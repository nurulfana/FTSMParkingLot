...
public class ParkingLot3 {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ParkingLot3 window = new ParkingLot3();
					window.frameSL.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frameSL = new JFrame();
		frameSL.getContentPane().setBackground(new Color(135, 206, 250));
		frameSL.setBounds(100, 100, 1104, 688);
		frameSL.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frameSL.setLocationRelativeTo(null);
		frameSL.getContentPane().setLayout(null);
		...
		frameSL.getContentPane().add(lblNewLabel);
		
		JTabbedPane tabbedPane = new JTabbedPane(JTabbedPane.TOP);
		tabbedPane.setBounds(10, 84, 1072, 558);
		frameSL.getContentPane().add(tabbedPane);
		...
		Parkbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg) {
				if(comboBox_1.getSelectedItem()!="CAR" || comboBox.getSelectedItem()!="STAFF") {
		...
		frameSL.getContentPane().add(Logoutbutton);
			}
	}
